...
(function (mod) {
  ...
  function autoCloseGT(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      var dontCloseTags =
        (typeof opt == 'object' && opt.dontCloseTags) ||
        (html && htmlDontClose);
      var indentTags =
        (typeof opt == 'object' && opt.indentTags) || (html && htmlIndent);
      ...
      replacements[i] = {
        ...
        text: '>' + (indent ? '\n\n' : '') + '</' + tagName + '>',
        newPos: indent
          ? CodeMirror.Pos(pos.line + 1, 0)
          : CodeMirror.Pos(pos.line, pos.ch + 1),
      };
    }

    var dontIndentOnAutoClose =
  ...
  function autoCloseCurrent(cm, typingSlash) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (mixed && inner.mode.name == 'javascript') {
        ...
        var context =
          inner.mode.xmlCurrentContext && inner.mode.xmlCurrentContext(state);
        if (
          !context ||
          (context.length &&
            closingTagExists(cm, context, context[context.length - 1], pos))
        )
          return CodeMirror.Pass;
    ...
    if (!dontIndentOnAutoClose) {
      for (var i = 0; i < ranges.length; i++)
        if (
          i == ranges.length - 1 ||
          ranges[i].head.line < ranges[i + 1].head.line
        )
