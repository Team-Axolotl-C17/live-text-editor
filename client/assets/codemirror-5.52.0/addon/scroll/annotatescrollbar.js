...
(function (mod) {
  ...
  function Annotation(cm, options) {
    ...
    this.buttonHeight =
      options.scrollButtonHeight || cm.getOption('scrollButtonHeight');
    this.annotations = [];
  ...
  Annotation.prototype.computeScale = function () {
    var cm = this.cm;
    var hScale =
      (cm.getWrapperElement().clientHeight -
  ...
  Annotation.prototype.redraw = function (compute) {
    ...
    function getY(pos, top) {
      ...
      if (
        (curLineObj.widgets && curLineObj.widgets.length) ||
    ...
    if (cm.display.barWidth)
      for (var i = 0, nextTop; i < anns.length; i++) {
        ...
        elt.style.cssText =
          ...
          Math.max(cm.display.barWidth - 1, 2) +
          'px; top: ' +
