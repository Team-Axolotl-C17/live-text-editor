...
(function (mod) {
  ...
  function wrapRange(cm, from, to, options) {
    ...
    for (var i = 0; i < lines.length; ++i) {
      ...
      if (i) {
        var firstBreak =
          curLine.length > column &&
          leadingSpace == spaceTrimmed &&
          findBreakPoint(curLine, column, wrapOn, killTrailing);
        // If this isn't broken, or is broken at a different point, remove old break
        if (
          !firstBreak ||
          firstBreak.from != oldLen ||
          firstBreak.to != oldLen + spaceInserted
        ) {
          changes.push({
            text: [spaceInserted ? ' ' : ''],
  ...
  CodeMirror.defineExtension('wrapParagraphsInRange', function (
    from,
    to,
    options
  ) {
