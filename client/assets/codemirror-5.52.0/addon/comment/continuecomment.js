    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (blockStart && mode.blockCommentContinue) {
        ...
        var end = line.lastIndexOf(
          ...
          pos.ch - mode.blockCommentEnd.length
        );
        ...
        if (
          ...
          (lineCmt &&
            (found = line.lastIndexOf(lineCmt, pos.ch - 1)) > -1 &&
            /\bcomment\b/.test(
              cm.getTokenTypeAt({ line: pos.line, ch: found + 1 })
            ))
        ) {
          // ...then don't continue it
        } else if (
          pos.ch >= blockStart.length &&
          (found = line.lastIndexOf(blockStart, pos.ch - blockStart.length)) >
            -1 &&
          found > end
        ) {
          ...
          if (nonspaceAfter(0, line) >= found) {
            ...
            insert = !cm.options.indentWithTabs
              ? repeat.call(' ', found)
      ...
      if (insert == null && lineCmt && continueLineCommentEnabled(cm)) {
        ...
        // continue only if the line starts with an optional space + line comment
          ...
          if (!insert) {
            ...
            insert =
              (nextFound > -1 && nonspaceAfter(0, next) >= nextFound) || null;
          }
          if (insert) {
            insert =
              line.slice(0, found) +
