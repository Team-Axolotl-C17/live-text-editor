...
(function (mod) {
  ...
  CodeMirror.TernServer.prototype = {
    ...
    request: function (cm, query, c, pos) {
      ...
      if (extraOptions)
        for (var prop in extraOptions) request.query[prop] = extraOptions[prop];
  ...
  function sendDoc(ts, doc) {
    ts.server.request(
      { files: [{ type: 'full', name: doc.name, text: docValue(ts, doc) }] },
      function (error) {
      ...
      function (error, data) {
        ...
        CodeMirror.on(obj, 'select', function (cur, node) {
          ...
          var content = ts.options.completionTip
            ? ts.options.completionTip(cur.data)
            : cur.data.doc;
          if (content) {
            tooltip = makeTooltip(
  ...
  function updateArgHints(ts, cm) {
    ...
    for (
      var line = cm.getCursor().line, e = Math.max(0, line - 9), found = false;
      line >= e;
      --line
    ) {
  ...
  function showArgHints(ts, cm, pos) {
    ...
    var tooltip = (ts.activeArgHints = makeTooltip(
      place.right + 1,
      place.bottom,
      tip,
      cm
  ...
  function findContext(doc, data) {
    ...
    var start = Pos(
      startLine,
      (before.length == 1 ? data.start.ch : doc.getLine(startLine).length) -
        before[0].length
    );
    ...
    for (
      var cur = startLine + 1;
      cur < doc.lineCount() && text.length < data.context.length;
      ++cur
    )
      text += '\n' + doc.getLine(cur);
  ...
  function atInterestingExpression(cm) {
    ...
    return /[\w)\]]/.test(
      cm.getLine(pos.line).slice(Math.max(pos.ch - 1, 0), pos.ch + 1)
    );
  }
  ...
  function rename(ts, cm) {
    ...
    dialog(cm, 'New name for ' + token.string, function (newName) {
      ts.request(
        cm,
        { type: 'rename', newName: newName, fullDocs: true },
        function (error, data) {
          if (error) return showError(ts, cm, error);
  ...
  function getFragmentAround(data, start, end) {
    ...
    if (
      ...
      for (endLine = end.line + 1; endLine < max; ++endLine) {
        var indent = CodeMirror.countColumn(
          doc.getLine(endLine),
          null,
          tabSize
        );
  ...
  function tempTooltip(cm, content, ts) {
    ...
    var tip = (cm.state.ternTooltip = makeTooltip(
      where.right + 1,
      where.bottom,
      content,
      cm
