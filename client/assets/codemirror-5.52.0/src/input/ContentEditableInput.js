...
export default class ContentEditableInput {
  ...
  init(display) {
    ...
    on(div, 'paste', (e) => {
      ...
      if (ie_version <= 11)
        setTimeout(
          operation(cm, () => this.updateFromDOM()),
          20
        );
    });
  ...
  receivedFocus() {
    ...
    else runInOp(this.cm, () => (input.cm.curOp.selectionChanged = true));
  ...
  pollContent() {
    ...
    let newText = cm.doc.splitLines(
      domTextBetween(cm, fromNode, toNode, fromLine, toLine)
    );
    let oldText = getBetween(
      cm.doc,
      Pos(fromLine, 0),
...
function locateNodeInLineView(lineView, node, offset) {
  ...
  for (
    let before = topNode.previousSibling, dist = offset;
    before;
    before = before.previousSibling
  ) {
    found = find(before, before.firstChild, -1);
    if (found) return badPos(Pos(found.line, found.ch + dist), bad);
    else dist += before.textContent.length;
  }
}
