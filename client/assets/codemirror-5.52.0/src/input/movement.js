  ...
  if (visually) {
    ...
    if (order) {
      ...
      if (part.level > 0 || cm.doc.direction == 'rtl') {
        ...
        ch = findFirst(
          (ch) => measureCharPrepared(cm, prep, ch).top == targetTop,
...
export function moveVisually(cm, line, start, dir) {
  ...
  let mv = (pos, dir) =>
    moveCharLogically(line, pos instanceof Pos ? pos.ch : pos, dir);
  let prep;
  ...
  let wrappedLineExtent = getWrappedLineExtent(
  ...
  let searchInVisualLine = (partPos, dir, wrappedLineExtent) => {
    let getRes = (ch, moveInStorageOrder) =>
      moveInStorageOrder
