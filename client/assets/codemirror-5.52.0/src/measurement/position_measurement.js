...
export function coordsChar(cm, x, y) {
  ...
  for (;;) {
    ...
    let collapsed = collapsedSpanAround(
      lineObj,
...
function wrappedLineExtent(cm, lineObj, preparedMeasure, y) {
  ...
  let begin = findFirst(
    (ch) => measureCharPrepared(cm, preparedMeasure, ch - 1).bottom <= y,
    end,
    0
  );
  end = findFirst(
    (ch) => measureCharPrepared(cm, preparedMeasure, ch).top > y,
    begin,
    end
  );
  return { begin, end };
...
function coordsCharInner(cm, lineObj, lineNo, x, y) {
  ...
  if (order) {
    let part = (cm.options.lineWrapping
      ? coordsBidiPartWrapped
      : coordsBidiPart)(cm, lineObj, lineNo, preparedMeasure, order, x, y);
    ltr = part.level != 1;
