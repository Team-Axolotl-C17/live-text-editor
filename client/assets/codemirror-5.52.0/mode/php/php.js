...
(function (mod) {
  ...
  function phpString_(stream, state, closing, escapes) {
    // "Complex" syntax
    if (
      (escapes !== false && stream.match('${', false)) ||
      stream.match('{$', false)
    ) {
      state.tokenize = null;
      return 'string';
    ...
    hooks: {
      ...
      '<': function (stream, state) {
        ...
        if ((before = stream.match(/<<\s*/))) {
          ...
          var delim = stream
            .current()
            .slice(before[0].length + (quoted ? 2 : 1));
    ...
    function (config, parserConfig) {
      ...
      function dispatch(stream, state) {
        ...
        if (!isPHP) {
          if (stream.match(/^<\?\w*/)) {
            ...
            if (!state.php)
              state.php = CodeMirror.startState(
                phpMode,
      ...
      return {
        startState: function () {
          ...
          var php = parserConfig.startOpen
            ? CodeMirror.startState(phpMode)
            : null;
          return {
            html: html,
            php: php,
