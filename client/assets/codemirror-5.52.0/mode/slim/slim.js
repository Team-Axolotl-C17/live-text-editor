...
(function (mod) {
  ...
  CodeMirror.defineMode(
    ...
    function (config) {
      ...
      var attributeNameRegexp = new RegExp(
        '^[:' + nameStartChar + '][:\\.' + nameChar + ']*(?=\\s*=)'
      );
      var wrappedAttributeNameRegexp = new RegExp(
      ...
      function sub(stream, state) {
        ...
        var subStream = new CodeMirror.StringStream(
      ...
      function startLine(stream, state) {
        var ch = stream.peek();
        if (ch == '<') {
          return (state.tokenize = startHtmlLine(state.tokenize))(
            stream,
            state
          );
        }
