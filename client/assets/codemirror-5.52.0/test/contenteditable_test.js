(function () {
  ...
  testCM(
    ...
    function (cm) {
      ...
      lineNode.parentNode.insertBefore(
        document.createElement('pre'),
        lineNode.nextSibling
      ).textContent = 'bar';
      cm.display.input.updateFromDOM();
      eq(cm.getValue(), 'one\ntwo\nfoo\nbar\nthree\nfour\n');
    },
  ...
  testCM(
    ...
    function (cm) {
      ...
      var text = findTextNode(cm, 'two'),
        startLine = lineElt(text);
      for (var i = 0; i < 3; i++)
  ...
  testCM(
    'force_redraw',
    function (cm) {
      findTextNode(cm, 'foo').parentNode.appendChild(
