      ...
      for (var ch = 0; ch <= 35; ch += 5) {
        for (var line = 0; line < 70; line += 5) {
          ...
          var pos = cm.coordsChar(
            { left: coords.left + 1, top: coords.top + 1 },
            sys
          );
...
testCM(
  ...
  function (cm) {
    ...
    eqCharPos(
      cm.coordsChar({ left: coords.right, top: coords.top + 2 }),
      Pos(0, 34)
    );
  },
  {
...
testCM('markTextMultiLine', function (cm) {
  function p(v) {
    return v && Pos(v[0], v[1]);
  }
  forEach(
    [
      { a: [0, 0], b: [0, 5], c: '', f: [0, 0], t: [2, 5] },
      { a: [0, 0], b: [0, 5], c: 'foo\n', f: [1, 0], t: [3, 5] },
    ...
    function (test) {
      ...
      var f = r.find();
      eqCursorPos(f && f.from, p(test.f));
      eqCursorPos(f && f.to, p(test.t));
    }
  );
});
...
testCM(
  ...
  function (cm) {
    ...
    var m1Pos = marker1.find(),
      m2Pos = marker2.find();
    eqCursorPos(m1Pos.from, Pos(0, 1));
    eqCursorPos(m1Pos.to, Pos(0, 3));
    eqCursorPos(m2Pos.from, Pos(0, 0));
    eqCursorPos(m2Pos.to, Pos(2, 1));
...
testCM(
  ...
  function (cm) {
    ...
    is(!m1.find());
    var m2 = cm.markText(Pos(0, 1), Pos(0, 2), {
...
testCM(
  'deleteSpanCollapsedInclusiveLeft',
  function (cm) {
    var from = Pos(1, 0),
      to = Pos(1, 1);
...
testCM(
  'markTextWithAttributes',
  function (cm) {
    function present() {
      var spans = cm.display.lineDiv.getElementsByTagName('span');
      for (var i = 0; i < spans.length; i++)
        if (
          spans[i].getAttribute('label') == 'label' &&
          spans[i].textContent == 'cdef'
        )
          return true;
    }
    var m = cm.markText(Pos(0, 2), Pos(0, 6), {
...
testCM(
  'bookmarkCursor',
  function (cm) {
    var pos01 = cm.cursorCoords(Pos(0, 1)),
      pos11 = cm.cursorCoords(Pos(1, 1)),
      pos20 = cm.cursorCoords(Pos(2, 0)),
      pos30 = cm.cursorCoords(Pos(3, 0)),
      pos41 = cm.cursorCoords(Pos(4, 1));
    cm.setBookmark(Pos(0, 1), {
      widget: document.createTextNode('←'),
      insertLeft: true,
    });
    cm.setBookmark(Pos(2, 0), {
      widget: document.createTextNode('←'),
      insertLeft: true,
    });
    cm.setBookmark(Pos(1, 1), { widget: document.createTextNode('→') });
    cm.setBookmark(Pos(3, 0), { widget: document.createTextNode('→') });
    var new01 = cm.cursorCoords(Pos(0, 1)),
      new11 = cm.cursorCoords(Pos(1, 1)),
      new20 = cm.cursorCoords(Pos(2, 0)),
      new30 = cm.cursorCoords(Pos(3, 0));
  ...
  function (cm) {
    ...
    function add(insertLeft) {
      for (var i = 0; i < 3; ++i) {
        ...
        node.innerHTML = 'X';
        ms.push(
          cm.setBookmark(Pos(0, 1), { widget: node, insertLeft: insertLeft })
        );
      }
    }
    var base1 = cm.cursorCoords(Pos(0, 1)).left,
      base4 = cm.cursorCoords(Pos(0, 4)).left;
    add(true);
...
testCM(
  ...
  function (cm) {
    ...
    var mark = cm.markText(Pos(0, 0), Pos(1, 1), {
      inclusiveLeft: true,
      inclusiveRight: true,
    });
    cm.setValue('foo');
...
testCM('scrollSnap', function (cm) {
  ...
  is(
    info.left == 0 &&
      info.top + 2 > info.height - cm.getScrollerElement().clientHeight,
...
testCM(
  ...
  function (cm) {
    ...
    var lineHeight =
      (cm.charCoords(Pos(99)).top - cm.charCoords(Pos(0)).top) / 100;
    cm.scrollTo(0, 0);
    ...
    for (var i = 0, e = selElt.length; i < e; ++i) {
      ...
      var atRight = box.right - outer.left > 0.8 * lineWidth;
      if (atLeft && atRight) {
        sawMiddle = true;
        is(box.bottom - box.top > 90 * lineHeight, 'middle high');
        is(width > 0.9 * lineWidth, 'middle wide');
      } else {
        is(width > 0.4 * lineWidth, 'top/bot wide enough');
        is(width < 0.6 * lineWidth, 'top/bot slim enough');
        if (atLeft) {
          sawBottom = true;
...
testCM('doubleScrollbar', function (cm) {
  ...
  is(
    wrap.offsetWidth - byClassName(wrap, 'CodeMirror-lines')[0].offsetWidth <=
      scrollbarWidth * 1.5
  );
});
...
testCM(
  ...
  function (cm) {
    ...
    m1.clear();
    var m1 = cm.markText(Pos(0, 0), Pos(1, 1), {
      collapsed: true,
      inclusiveLeft: true,
    });
    var m2 = cm.markText(Pos(1, 1), Pos(2, 0), {
      collapsed: true,
      inclusiveRight: true,
    });
    eqCharPos(cm.coordsChar(pos_1_3), Pos(3, 3));
...
testCM(
  'collapsedRangeBetweenLinesSelected',
  function (cm) {
    if (cm.getOption('inputStyle') != 'textarea') return;
    ...
    for (var i = 0, w = 0; i < selElts.length; i++) w += selElts[i].offsetWidth;
    is(w > 0);
  },
  { value: 'one\ntwo' }
);
...
testCM('randomCollapsedRanges', function (cm) {
  ...
  cm.operation(function () {
    for (var i = 0; i < 200; i++) {
      ...
      if (i % 4)
        try {
          cm.markText(start, Pos(start.line + 2, 1), { collapsed: true });
        } catch (e) {
          if (!/overlapping/.test(String(e))) throw e;
        }
      else
...
testCM(
  'hiddenLinesAutoUnfold',
  function (cm) {
    var range = foldLines(cm, 1, 3, true),
      cleared = 0;
    CodeMirror.on(range, 'clear', function () {
      cleared++;
    });
...
testCM('hiddenLinesSelectAll', function (cm) {
  // Issue #484
  addDoc(cm, 4, 20);
  foldLines(cm, 0, 10);
  foldLines(cm, 11, 20);
...
testCM('clickFold', function (cm) {
  ...
  is(
    foundOn.ch <= 5 || foundOn.ch >= 10,
    'Position is not inside the folded range'
  );
});
...
testCM('badNestedFold', function (cm) {
  ...
  var caught;
  try {
    cm.markText(Pos(0, 1), Pos(0, 3), { collapsed: true });
  } catch (e) {
    caught = e;
  }
...
testCM(
  'nestedFoldOnSide',
  function (cm) {
    var m1 = cm.markText(Pos(0, 1), Pos(2, 1), {
      collapsed: true,
      inclusiveRight: true,
    });
    ...
    var m4 = cm.markText(Pos(2, 0), Pos(2, 1), {
      collapse: true,
      inclusiveRight: true,
    });
    m1.clear();
...
testCM(
  ...
  function (cm) {
    ...
    var cur0 = cm.cursorCoords(Pos(0, 0)),
      cur1 = cm.cursorCoords(Pos(0, 10));
    is(cur0.top < cur1.top);
    is(cur0.bottom < cur1.bottom);
    var curL = cm.cursorCoords(Pos(0, 6)),
...
testCM(
  ...
  function (cm) {
    ...
    w.innerHTML =
      'and now the widget is really really long all of a sudden and a scrollbar is needed';
    m.changed();
    var hScroll = byClassName(
...
testCM(
  'inlineWidget',
  function (cm) {
    var w = cm.setBookmark(Pos(0, 2), {
      widget: document.createTextNode('uu'),
    });
    cm.setCursor(0, 2);
    CodeMirror.commands.goLineDown(cm);
...
testCM(
  ...
  function (cm) {
    ...
    var wrap = cm.getWrapperElement(),
      h0 = wrap.offsetHeight;
    var doc = 'xxx xxx xxx xxx xxx';
    cm.setValue(doc);
    for (
      var step = 10, w = cm.charCoords(Pos(0, 18), 'div').right;
      ;
      w += step
    ) {
      cm.setSize(w);
    ...
    forEach(
      ...
      function (pos) {
        ...
        eqCharPos(
          pos,
          cm.coordsChar({ left: coords.left + 2, top: coords.top + 5 })
        );
      }
    );
  },
...
testCM(
  ...
  function (cm) {
    cm.setSize(null, 'auto');
    var inner = byClassName(cm.getWrapperElement(), 'CodeMirror-lines')[0]
      .firstChild;
    var lh = inner.offsetHeight;
    for (
      var step = 10, w = cm.charCoords(Pos(0, 7), 'div').right;
      ;
      w += step
    ) {
      cm.setSize(w);
    ...
    is(endPos.top > lh * 0.8, 'not at top');
    is(endPos.left > w - 20, 'at right');
    endPos = cm.charCoords(Pos(0, 18));
    eqCursorPos(
      cm.coordsChar({ left: endPos.left, top: endPos.top + 5 }),
      Pos(0, 18, 'before')
    );
    ...
    eqCursorPos(
      cm.coordsChar({ left: wrapPos.left + 10, top: wrapPos.top }),
      Pos(0, 9, 'before')
    );
  },
...
testCM(
  'measureEndOfLineBidi',
  function (cm) {
    eqCursorPos(
      cm.coordsChar({ left: 5000, top: cm.charCoords(Pos(0, 0)).top }),
      Pos(0, 8, 'after')
    );
  },
  { value: 'إإإإuuuuإإإإ' }
...
testCM(
  ...
  function (cm) {
    ...
    cm.setCursor(39);
    var wrap = cm.getWrapperElement(),
      bar = byClassName(wrap, 'CodeMirror-vscrollbar')[0];
    is(
      bar.offsetHeight < wrap.offsetHeight,
      'vertical scrollbar limited by horizontal one'
    );
    var cursorBox = byClassName(
      wrap,
      'CodeMirror-cursor'
    )[0].getBoundingClientRect();
    var editorBox = wrap.getBoundingClientRect();
    is(
      cursorBox.bottom < editorBox.top + cm.getScrollerElement().clientHeight,
      'bottom line visible'
    );
  },
  { lineNumbers: true }
...
testCM(
  'moveVstuck',
  function (cm) {
    var lines = byClassName(cm.getWrapperElement(), 'CodeMirror-lines')[0]
        .firstChild,
      h0 = lines.offsetHeight;
    var val =
      'fooooooooooooooooooooooooo baaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaar\n';
    cm.setValue(val);
    ...
    is(
      cm.cursorCoords(null, 'local').top < h0,
      'cursor is in first visual line'
    );
  },
  { lineWrapping: true },
  ie_lt8 || opera_lt10
...
testCM(
  ...
  function (cm) {
    ...
    eqCharPos(
      cm.coordsChar({ left: p0.right - 5, top: p0.top + 5 }),
      Pos(0, 1)
    );
  },
...
testCM(
  ...
  function (cm) {
    ...
    CodeMirror.commands.testCommand = function () {
      outcome = 'tc';
    };
    CodeMirror.commands.goTestCommand = function () {
      outcome = 'gtc';
    };
...
testCM(
  'verticalMovementCommandsWrapping',
  function (cm) {
    cm.setSize(120);
...
testCM(
  'verticalMovementCommandsSingleLine',
  function (cm) {
  ...
  function (cm) {
    ...
    forEach(
      [
        ...
        'خ ة ق',
        '<img src="/בדיקה3.jpg">',
        'يتم السحب في 05 فبراير 2014',
      ],
      function (line) {
        cm.setValue(line + '\n');
        cm.execCommand('goLineStart');
        var cursors = byClassName(
          cm.getWrapperElement(),
          'CodeMirror-cursors'
        )[0];
        var cursor = cursors.firstChild;
        var prevX = cursor.offsetLeft,
          prevY = cursor.offsetTop;
...
testCM('lineChangeEvents', function (cm) {
  addDoc(cm, 3, 5);
  var log = [],
    want = ['ch 0', 'ch 1', 'del 2', 'ch 0', 'ch 0', 'del 1', 'del 3', 'del 4'];
  for (var i = 0; i < 5; ++i) {
    CodeMirror.on(
      cm.getLineHandle(i),
      'delete',
  ...
  for (var i = 0; i < log.length; ++i) eq(log[i], want[i]);
});
...
testCM('scrollEntirelyToRight', function (cm) {
  ...
  var wrap = cm.getWrapperElement(),
    cur = byClassName(wrap, 'CodeMirror-cursor')[0];
...
testCM('lineWidgetChanged', function (cm) {
  ...
  function w() {
    ...
    var barelyUnderHalfWidthHtml =
      '<div style="display: inline-block; height: 1px; width: ' +
      (285 - halfScrollbarWidth) +
      'px;"></div>';
    var barelyOverHalfWidthHtml =
      '<div style="display: inline-block; height: 1px; width: ' +
      (305 - halfScrollbarWidth) +
      'px;"></div>';
    node.innerHTML =
      new Array(3).join(barelyUnderHalfWidthHtml) +
      new Array(3).join(barelyOverHalfWidthHtml);
    node.style.cssText =
      'background: yellow;font-size:0;line-height: ' +
      expectedWidgetHeight / expectedLinesInWidget +
      'px;';
    return node;
  }
...
testCM(
  ...
  function (cm) {
    ...
    cm.addKeyMap({
      ...
      'Shift-LeftDoubleClick': function () {
        fired.push('b');
      },
      'Shift-LeftTripleClick': function () {
        fired.push('c');
      },
    });

    function send(button, mods) {
      mouseDown(cm, button, Pos(1, 2), mods);
    }
...
testCM(
  'configureMouse',
  function (cm) {
    cm.setOption('configureMouse', function () {
      return { unit: 'word' };
    });
...
testCM(
  ...
  function (cm) {
    forEach(
      [
        { from: Pos(0, 0), to: Pos(0, 1, 'before'), by: 1 },
        ...
        { from: Pos(1, 15), to: Pos(2, 0, 'after'), by: 50, hitSide: true },
      ],
      function (t) {
...
testCM('beforeSelectionChange', function (cm) {
  ...
  cm.on('beforeSelectionChange', function (cm, obj) {
    obj.update([
      {
        anchor: notAtEnd(cm, obj.ranges[0].anchor),
    ...
    CodeMirror.defineMode('test_mode', function () {
      return {
        token: function (stream) {
          ...
          if (stream.match(/^<[^>]*>/))
            return '  span  line-line  line-background-bg  ';
          stream.match(/^\s+|^\S+/);
        },
      };
    });
...
testCM(
  'selectionHistoryNonOverlapping',
  function (cm) {
...
testCM(
  'selChangeInOperationDoesNotSplit',
  function (cm) {
    for (var i = 0; i < 4; i++) {
      cm.operation(function () {
        cm.replaceSelection('x');
...
testCM(
  'alwaysMergeSelEventWithChangeOrigin',
  function (cm) {
...
var getChar = function (noExtending) {
  var res;
  do {
    res = String.fromCharCode(Math.floor(Math.random() * 0x8ac));
  } while (
    [0x90].indexOf(res.charCodeAt(0)) != -1 ||
    (noExtending && extendingChars.test(res))
  );
  return res;
};
var getString = function (n) {
  var res = getChar(true);
  while (--n > 0) res += getChar();
  return res;
};
...
function testMoveBidi(str) {
  testCM(
    ...
    function (cm) {
      if (
        cm.getOption('inputStyle') != 'textarea' ||
        !cm.getOption('rtlMoveVisually')
      )
        return;
      ...
      var steps =
        str.length -
        countIf(str.split(''), function (ch) {
          return extendingChars.test(ch);
        });
      var lineBreaks = {};
      lineBreaks[
        6 -
          countIf(str.substr(0, 5).split(''), function (ch) {
            return extendingChars.test(ch);
          })
      ] = 'w';
      if (str.indexOf('\n') != -1) {
      ...
      var prevCoords = cm.cursorCoords(),
        coords;
      for (var i = 0; i < steps; ++i) {
        ...
        if (
          ...
          i <= 12 &&
          !lineBreaks[i] &&
          coords.left < prevCoords.left &&
          coords.top > prevCoords.top
        ) {
        ...
        if (!lineBreaks[i]) {
          is(
            coords.left > prevCoords.left,
            'In step ' + i + ", cursor didn't move right"
          );
          eq(
            coords.top,
            prevCoords.top,
            'In step ' + i + ', cursor moved out of line'
          );
        } else {
      ...
      eq(
        coords.left,
        prevCoords.left,
        'Moving ' + steps + " steps right didn't reach the end"
      );
      eq(
        coords.top,
        prevCoords.top,
      ...
      for (i = steps - 1; i >= 0; --i) {
        ...
        if (!(lineBreaks[i] == 'n' || lineBreaks[i + 1] == 'w')) {
          is(
            coords.left < prevCoords.left,
            'In step ' + i + ", cursor didn't move left"
          );
          eq(
            coords.top,
            prevCoords.top,
            'In step ' + i + ', cursor is not at the same line anymore'
          );
        } else {
          is(coords.left > prevCoords.left, i);
          is(coords.top < prevCoords.top, i);
      ...
      eq(
        coords.left,
        prevCoords.left,
        'Moving ' + steps + " steps left didn't reach the beginning"
      );
      eq(
        coords.top,
        prevCoords.top,
...
function testCoordsWrappedBidi(str) {
  testCM(
    ...
    function (cm) {
      ...
      var top = cm.charCoords(Pos(0, 0)).top,
        lastTop;
...
testCM(
  ...
  function (cm) {
    ...
    is(blocks.length >= 2);
    is(blocks.length <= 3);
    var boxTop = blocks[0].getBoundingClientRect(),
      boxBot = blocks[blocks.length - 1].getBoundingClientRect();
    is(boxTop.left > cm.charCoords(Pos(0, 1)).right);
    is(boxBot.right < cm.charCoords(Pos(0, cm.getLine(0).length - 2)).left);
  },
  {
    value:
      '<p>مفتي11 تم تضمينهفتي تم تضمينها فتي تفتي تم تضمينها فتي تفتي تم تضمينها فتي تفتي تم تضمينها فتي تا فت10ي ت</p>',
    lineWrapping: true,
  }
);
...
testCM(
  ...
  {
    value:
      '  في تطبيق السمات مرة واحدة https://github.com/codemirror/CodeMirror/issues/4878#issuecomment-330550964على سبيل المثال <code>"foo bar"</code>\n' +
      '  سيتم تعيين',
    direction: 'rtl',
    lineWrapping: true,
  }
